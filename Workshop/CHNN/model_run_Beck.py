...
from CHNN import to_learning, performance, Coastal_Model as cm
...
def get_input_data(station, variables, ML, input_dir, resample, resample_method, batch,
                   scaler_type, year, n_ncells, mask_val, 
                   loop, tt_value, frac_ens, NaN_threshold, loss,
                   logger, hyper_opt=False):
    ML_list = [ML]
    ...
    for i in range(loop):
        ...
        if hyper_opt:
            ...
            sherpa_output = None
    return test_X, test_y, train_X, train_y, val_X, val_y, n_train, result_all, sherpa_output 
...
def ensemble(station, variables, ML, tt_value, input_dir, resample, resample_method, scaler_type,
    ...
    for ML in ML_list:
        ...
        for i in range(loop):
            ...
            # design network
            model  =  cm.Coastal_Model(loss, n_layers, neurons, activation, train_X, train_y, dropout, drop_value, 
                                    hyper_opt, validation, test_X, test_y,
                                    val_X, val_y, optimizer, epochs, batch, verbose, model_dir, name_model, ML, filters, variables, batch_normalization, sherpa_output, logger,
            ...
            result_all['train_loss'][i] = model.train_loss
            result_all['test_loss'][i] = model.test_loss
