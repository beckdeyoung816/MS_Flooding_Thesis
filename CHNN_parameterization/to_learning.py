...
def reframe_scale(df, timesteps, scaler_type='MinMax', year = 'last', prior=False, scaler_op=True):
    ...
    if year == 'random':
        dates, _ = draw_sample(df, 'residual', timesteps, threshold=0)
    else:
        dates = df.iloc[-timesteps:].index.values
        
    i_dates = np.array([df.index.get_loc(date) for date in dates])
    all_index = df.reset_index().index.values
...
#        train = values[:n_train_hours, :]
    ...
    if prior:
        ...
        columns = ['var{}(t)'.format(i + 1) for i in range(len(df.columns) - 1)]
...
def select_ensemble(df, col, ML, batch, tt_value=0.7, frac_ens = 0.5, mask_val=-999):
    df = df.copy()
...
def draw_sample(df, col, timesteps, threshold=0):
    """ 
...
def prescreening(station_list, input_dir, batch, threshold=0):
    ...
    for station in station_list:
        ...
        try:
            ...
            available = True if cons_years >= 1 and cons_sequences >= 26 else False
